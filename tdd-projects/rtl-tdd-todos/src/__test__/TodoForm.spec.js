...
describe('<TodoForm />', () => {
  ...
  it('has input and a button', () => {
    ...
    expect(input).toBeTruthy(); // 해당 값이 trythy 한 값인지 확인
  ...
  it('calls onInsert and clears input', () => {
    ...
    const { input, button } = setup({ onInsert }); // props 가 필요할 땐 이렇게 직접 파라미터로 전달
    // 수정하고
    fireEvent.change(input, {
      target: {
        value: 'TDD 배우기'
    ...
    expect(input).toHaveAttribute('value', ''); // input 이 비워져야함
  });
});